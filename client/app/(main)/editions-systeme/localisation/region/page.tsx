...
import { ParametrageService, ProgrammationDTO, RegionDTO, SujetDTO } from '@/demo/service/ParametrageService';
...
const Crud = () => {
    ...
    var id_rg = useRef(null); // <== même chose pour l'ID du candidat
    ...
    const editProduct = (region) => {
        setProductDialog(true);

        const rgFormatted = {
            ...region
        };
        (id_rg.current = region.id), console.log(id_rg);
        formik.setValues(rgFormatted);
        is_update.current = true;
        console.log(is_update);
        console.log(rgFormatted);
    };
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button severity="success" label="Ajouter une région" icon="pi pi-plus" className="mr-2" onClick={openNew} />
        ...
        return (
            <>
                ...
                <Button
                                    icon="pi pi-pencil"
                                    rounded
                                    tooltip="Modifier l'intitulé de la région"
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Listing des région (s)</h5>
        ...
        onSubmit: async (values, { setSubmitting, resetForm }) => {
            ...
            try {
                if (is_update.current === false) {
                    console.log('POST');
                    const response = await ParametrageService.createRegion(regionDTO);
                    console.log('Data créé:', response.data);
                    toast.current.show({ severity: 'success', summary: 'Office du Bac', detail: 'Prog créé avec succès', life: 4000 });
                    resetForm();
                }
                if (is_update.current === true) {
                    console.log('PUT');
                    const response = await ParametrageService.updateRegion(id_rg, regionDTO);
                    console.log('Data créé:', response.data);
                    toast.current.show({ severity: 'success', summary: 'Office du Bac', detail: 'Prog créé avec succès', life: 4000 });
                    resetForm();
                }
            ...
            <div className="grid crud-demo">
                <div className="col-12">
                    <div className="card">
                        ...
                        <Dialog visible={productDialog} style={{ width: '700px' }} header="Gestion d'une région" modal className="p-fluid" onHide={hideDialog}>
                            <form onSubmit={formik.handleSubmit}>
                                <div className="formgrid grid py-3">
                                    <div className="field col-12">
                                        <label htmlFor="spec_id">* Intitulé de la région</label>
                                        <InputText
                                            id="name"
                                            name="name"
                                            value={formik.values.name}
                                            onChange={formik.handleChange}
                                            placeholder="Saisir l'intitulé de la région"
                                ...
                                <div className="formgrid grid py-3">
                                    <div className="field col-12">
                                        <div>
                                            <Button severity="success" label="Ajouter une région" className="mr-2" type="submit" />
                                            {/* <Button severity="danger" label="Delete" icon="pi pi-trash" onClick={confirmDeleteSelected} disabled={!selectedProducts || !selectedProducts.length} /> */}
                                        </div>
                                    </div>
                                </div>
