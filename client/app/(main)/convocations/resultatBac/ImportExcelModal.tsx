...
export default function ImportExcelModal({ open, onClose, onSuccess }: Props) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Transformer les données si nécessaire pour correspondre à l'affichage attendu
      const extendedResult: ExtendedImportResult = {
        ...data,
        errorRows: data.warnings?.filter(w => w.includes("Erreur")).length || 0,
        errors: (data.warnings || [])
          .filter(w => w.includes("Erreur") || w.includes("ligne"))
          .map((w, idx) => ({ row: idx + 2, message: w })),
        duplicates: [],
        ignored: (data.warnings || [])
          .filter(w => w.includes("ignoré") || w.includes("doublon"))
          .map((w, idx) => ({ row: idx + 2, reason: w })),
        ignoredCount: data.warnings?.length || 0,
        missingEtablissementsPrincipaux: {},
        missingCentresEcrit: {},
        missingCentresEPS: {},
      };
      ...
      // Rafraîchir la liste après import réussi
      await fetchAll();
      onSuccess?.();
    } catch (err: any) {
      clearInterval(interval);
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div>
        ...
        <p style={{ margin: 0, fontSize: 12, color: "#888" }}>
          Fichier .xlsx ou .xls uniquement
        </p>
      </div>
    </div>
  );
      ...
      <div>
        ...
        {state === "success" && result && (
          <>
            ...
            >
              <StatBadge label="Importés" value={result.nouveaux + result.modifies} color="#16a34a" />
              <StatBadge label="Nouveaux" value={result.nouveaux} color="#22c55e" />
              <StatBadge label="Modifiés" value={result.modifies} color="#f59e0b" />
              <StatBadge label="Inchangés" value={result.inchanges} color="#6b7280" />
              <StatBadge label="Jury inconnu" value={result.juryIntrouvable} color="#ef4444" />
            </div>
            ...
            {result.warnings && result.warnings.length > 0 && (
              <>
                ...
                >
                  ...
                  {activeTab === "duplicates" && result.duplicates && result.duplicates.length > 0 && (
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                      <thead>
                  ...
                  {activeTab === "ignored" && result.ignored && result.ignored.length > 0 && (
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                      <thead>
                  ...
                  {activeTab === "missing" && (
                    <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
                      {missingEntries(result.missingEtablissementsPrincipaux).length > 0 && (
                        <div>
                          <p style={{ margin: "0 0 6px", fontSize: 11, fontWeight: 600, color: "#0ea5e9", textTransform: "uppercase", letterSpacing: ".5px" }}>
