...
const ValidationCandidat = () => {
    ...
    const loadCandidats = useCallback(async () => {
        ...
        try {
            ...
            const response = await CandidatureService.getSummarizeOperationsByOps(prog?.edition, user?.login);
    ...
    //                                                         <label htmlFor="etablissement"><h6 className="m-0">Choisir un receptionniste</h6></label>
    //                                                         <Dropdown
    //                                                             showClear
    //                                                             id="receptionniste"
    //                                                             name="receptionniste"
    //                                                             options={receptionnistes}
    //                                                             optionLabel="username"
    //                                                             placeholder="Choisir un receptionniste"
    //                                                             value={receptionniste}
    //                                                             onChange={(e) => setReceptionniste(e.value)}
    //                                                             filter
  ...
  session: number;
    ...
    const exportExcel2 = () => {
            import('xlsx').then((xlsx) => {
                ...
                saveAs(blob, `Listing des établissements receptionnés par ${user?.login} pour le BAC ${prog?.edition}.xlsx`);
            });
        };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <div className="flex flex-wrap gap-3">
                 <div className="flex flex-wrap gap-3">
                                                <h4>Listing des établissements receptionnés</h4>
                                            </div>
                                            ...
                                            {(prog && (
                                                <div className="flex gap-3">
                                                    <Button
                                                        ...
                                                        label="Exporter la liste des établissements receptionnés"
                                                    />
                                                </div>
                                            ))}
    ...
    formik.values.matiere3?.name
            ...
            const reprTemplate = (rowData) => {
                return (
                    <>
                        <b>{rowData.representative}</b><br/>
                    </>
                );
            };

            const nbCdtTemplate = (rowData) => {
                return (
                    <>
                        <b>{rowData.count_5000}</b><br/>
                    </>
                );
            };
...
const selectedOption = options.find(option => option.name === formik.values.eprFacListB) || null;
    ...
    return (
        <ProtectedRoute allowedRoles={['SCOLARITE', 'ADMIN', 'RECEPTIONNISTE']}>
                    ...
                    <div className="card">
                        ...
                        {/* <Toolbar className="mb-4" left={leftToolbarTemplate} right={rightToolbarTemplate}></Toolbar> */}
                                ...
                                <DataTable
                                                                                                        ...
                                                                                                        header={header}
                                                                                                    >
                                                                                                                <Column field="representative" sortable header="Prénom (s) et Nom du mandataire" body={reprTemplate} headerStyle={{ minWidth: '10rem' }} />
                                                                                                                <Column field="etablissement.name" sortable header="Etablissement" body={SBodyTemplate} headerStyle={{ minWidth: '5rem' }} />
                                                                                                                <Column header="Nombre de cdts." body={nbCdtTemplate} headerStyle={{ minWidth: '5rem' }} />
