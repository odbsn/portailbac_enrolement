...
export default function AuthentificationCandidat() {
  ...
  const isFormValid = useMemo(() => {
    ...
    const hasCodeError = !!(authErrors.codeEtablissement || errors.codeEtablissement);
    ...
    return !hasCodeError && !hasTableError && !hasJourError && !hasMoisError && !hasAnneeError;
  ...
  const getMaxDays = () => {
    return getMaxDaysFromHook(formData.dateNaissance.mois, formData.dateNaissance.annee);
  };
  ...
  setFormData(prev => ({
    ...
    codeEtablissement: name === 'codeEtablissement' ? sanitizedValue : formData.codeEtablissement,
    numeroTable: name === 'numeroTable' ? sanitizedValue : formData.numeroTable,
  ...
  const handleSubmit = async () => {
    ...
    if (!isTurnstileVerified || !turnstileToken) {
      toastRef.current?.show({
        ...
        detail: turnstileError || "Veuillez compléter la vérification de sécurité",
        life: 3000,
      });
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        safeNavigate("/convocations/candidats");
      }, 1500);
    } catch (error) {
    ...
    <>
            ...
            >
              ...
              <div className="card-body">
                ...
                <motion.div
                  ...
                  transition={{ delay: 0.6, duration: 0.5 }}
                >
                  <label htmlFor="codeEtablissement" className="font-semibold block">
                  ...
                  <InputText
                    ...
                    onBlur={() => handleAuthBlur("codeEtablissement", {
                      codeEtablissement: formData.codeEtablissement,
                      numeroTable: formData.numeroTable,
                      dateNaissance: formData.dateNaissance,
                    })}
                    className={`w-full ${errors.codeEtablissement ? "p-invalid" : ""}`}
                    placeholder="Ex: LSLL"
                    disabled={isLoading}
                  />
                  <ValidationTooltip
                    message={errors.codeEtablissement || authErrors.codeEtablissement || ""}
                    isVisible={!!(errors.codeEtablissement || (authTouched.codeEtablissement && authErrors.codeEtablissement))}
                  />
                </motion.div>
                ...
                >
                  ...
                  <InputText
                    ...
                    onChange={handleInputChange}
                    onBlur={() => handleAuthBlur("numeroTable", {
                      codeEtablissement: formData.codeEtablissement,
                      numeroTable: formData.numeroTable,
                      dateNaissance: formData.dateNaissance,
                    })}
                    className={`w-full ${errors.numeroTable ? "p-invalid" : ""}`}
                    placeholder="Ex: 106610"
                    disabled={isLoading}
                  />
                  <ValidationTooltip
                    message={errors.numeroTable || authErrors.numeroTable || ""}
                    isVisible={!!(errors.numeroTable || (authTouched.numeroTable && authErrors.numeroTable))}
                  />
                </motion.div>
                ...
                >
                  ...
                  <div className="date-picker-container">
                    <div className="date-column">
                      ...
                      <input
                        type="text"
                        className={`custom-date-input ${jourError ? "p-invalid" : ""}`}
                    ...
                    <div className="date-column">
                      ...
                      <input
                        type="text"
                        className={`custom-date-input ${moisError ? "p-invalid" : ""}`}
                    ...
                    <div className="date-column">
                      ...
                      <input
                        type="text"
                        className={`custom-date-input ${anneeError ? "p-invalid" : ""}`}
      ...
      <style jsx global>{`
        ...
        @media (max-width: 768px) {
          ...
          .page-subtitle { font-size: 1.25rem !important; }
          .candidat-card { margin-top: -130px; }
        }
        @media (max-width: 640px) {
          .page-title { font-size: 1.1rem; }
          .page-subtitle { font-size: 0.75rem; }
          .card-header { padding: var(--spacing-md) var(--spacing-md) var(--spacing-xs) var(--spacing-md); }
          .card-body { padding: var(--spacing-md); }
          .candidat-card { max-width: 100%; margin-top: -120px; }
          .card-footer { padding: 0 var(--spacing-md) var(--spacing-md) var(--spacing-md); }
          .date-column { width: auto; min-width: 0; }
          .field { margin-bottom: var(--spacing-md); }
        }
        @media (max-width: 480px) {
          .page-title { font-size: 1rem; }
          .page-subtitle { font-size: 0.7rem; }
          .candidat-card { margin-top: -100px; }
          .date-picker-container { gap: var(--spacing-xs); }
        }
      `}</style>
    </>
  );
