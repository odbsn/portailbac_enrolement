...
const ValidationCandidat = () => {
    ...
    const loadCandidats = useCallback(async () => {
        ...
        try {
            if (!receptionniste || !edition) {
            setLoading(false);
            return;
            }

            console.log(receptionniste, edition);

            const response = await CandidatureService.getSummarizeOperationsByScol(edition?.edition, receptionniste?.username);
  ...
  session: number;
    ...
    const exportExcel2 = () => {
                import('xlsx').then((xlsx) => {
                    ...
                    saveAs(blob, `Listing des établissements receptionnés par ${receptionniste?.firstname} ${receptionniste?.lastname} pour le BAC ${prog?.edition}.xlsx`);
                });
            };
    ...
    const SBodyTemplate = (rowData) => {
                return (
                    <>
                        ...
                        >Mandataire : {rowData?.representative}</span>
                        <span style={{
                                display: "inline-block",
                                backgroundColor: "OrangeRed",
                                color: "white",
