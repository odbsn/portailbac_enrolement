...
isExportingPdf: boolean;
  ...
  resetFilters: () => void;
  clearError: () => void;
 exportPdfBySerie: (serieCode: string) => Promise<Blob | null>;
  exportZipBySeries: () => Promise<Blob | null>;
  isExportingPdfBySerie: boolean;
  isExportingZipBySeries: boolean;
}
...
const initialState = {
  candidats: [],
  isExportingPdfBySerie: false,
isExportingZipBySeries: false,
...
export const useCandidatEtabStore = create<CandidatState>()((set, get) => ({
  ...initialState,

  // ==================== FETCH CANDIDATS (PAGINÉ) ====================
exportPdfBySerie: async (serieCode: string) => {
  console.log(`📄 exportPdfBySerie - Génération PDF pour série: ${serieCode}`);
  
  set({ isExportingPdfBySerie: true, error: null });
  
  try {
    const response = await axiosInstance.get(
      `candidats/pdf/by-serie/${serieCode}`,
      {
        responseType: "blob",
      }
    );
    
    set({ isExportingPdfBySerie: false });
    
    return response.data;
  } catch (error: any) {
    console.error("❌ Erreur export PDF par série:", error);
...
exportZipBySeries: async () => {
  console.log("📦 exportZipBySeries - Génération ZIP par série...");
  
  set({ isExportingZipBySeries: true, error: null });
  
  try {
    const response = await axiosInstance.get(
      `candidats/zip/by-series`,
      {
        responseType: "blob",
      }
    );
    
    set({ isExportingZipBySeries: false });
  ...
  exportCandidats: async (filters?: FilterParams) => {
    console.log("📊 exportCandidats - Export des candidats...");
    set({ isLoading: true, error: null });

    try {
      const currentFilters = { ...get().filters, ...filters };
      const { page, size, ...exportFilters } = currentFilters;
      const queryString = buildParams(exportFilters);

      const response = await axiosInstance.get(
        `candidats/me/export?${queryString}`,
        {
          responseType: "blob",
        },
      );
