...
import { DepartementDTO, ParametrageService, ProgrammationDTO, RegionDTO, SujetDTO } from '@/demo/service/ParametrageService';
...
const Crud = () => {
    ...
    var id_dep = useRef(null); // <== même chose pour l'ID du candidat
    ...
    useEffect(() => {
        ParametrageService.getRegions().then((response) => {
    ...
    const loadData = async () => {
        ...
        setError(null);
        try {
            ParametrageService.getDepartements()
                .then((response) => {
                    //console.log("📦 Séries chargées :", data);
    ...
    const editProduct = (departement) => {
        setProductDialog(true);

        const dpFormatted = {
            ...departement
        };
        (id_dep.current = departement.id), console.log(id_dep);
        formik.setValues(dpFormatted);
        is_update.current = true;
        console.log(is_update);
        console.log(dpFormatted);
    };
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button severity="success" label="Ajouter un département" icon="pi pi-plus" className="mr-2" onClick={openNew} />
        ...
        return (
            <>
                <Button
                                                    icon="pi pi-pencil"
                                                    rounded
                                                    tooltip="Modifier l'intitulé du département"
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Listing des département (s)</h5>
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            region: Yup.object().required('Champ requis')
        }),

        onSubmit: async (values, { setSubmitting, resetForm }) => {
            const departementDTO: DepartementDTO = {
                name: values.name,
            ...
            try {
                if (is_update.current === false) {
                    console.log('POST');
                    const response = await ParametrageService.createDepartement(departementDTO);
                ...
                if (is_update.current === true) {
                    console.log('PUT');
                    const response = await ParametrageService.updateDepartement(id_dep, departementDTO);
                    console.log('Data créé:', response.data);
                            ...
                            <form onSubmit={formik.handleSubmit}>
                                <div className="formgrid grid">
                                    <div className="field col-6">
                                        <label htmlFor="spec_id">* Intitulé du département</label>
                                        <InputText
                                            id="name"
                                            name="name"
                                    ...
                                    <div className="field col-6">
                                        <label htmlFor="lastname">* Region</label>
                                        <Dropdown
                                            id="region"
                                            name="region"
                                        ...
                                        />
                                        {formik.touched.region && typeof formik.errors.region === 'string' && <small className="p-error">{formik.errors.region}</small>}
                                    </div>
                                </div>
                                ...
                                <div className="formgrid grid">
                                    <div className="field col-6">
                                        <div>
                                            <Button severity="success" label="Ajouter un département" className="mr-2" type="submit" />
                                            {/* <Button severity="danger" label="Delete" icon="pi pi-trash" onClick={confirmDeleteSelected} disabled={!selectedProducts || !selectedProducts.length} /> */}
                                        </div>
                                    </div>
                                </div>
