...
// ─── Carte de résultat pour un fichier (accordéon) ────────────────────────────
...
export default function ImportExcelMultipleModal({ open, onClose, onSuccess }: Props) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const data = await importExcelMultiple(files);
      clearInterval(interval);
      setProgress(100);
      setResults(data);
      setState("success");
  ...
  // ── Totaux agrégés sur tous les fichiers ──
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div>
        ...
        <p style={{ margin: 0, fontSize: 12, color: "#888" }}>
          Sélection multiple — .xlsx ou .xls
        </p>
      </div>
    </div>
  );
        ...
        {state === "idle" && (
          <>
            ...
            <div
              ...
              style={{
                border: `2px dashed ${dragging ? "#3b82f6" : files.length > 0 ? "#3b82f6" : "#e5e7eb"}`,
                borderRadius: 12,
                padding: "24px 20px",
                textAlign: "center",
                cursor: "pointer",
                background: dragging || files.length > 0 ? "#eff6ff" : "#fafafa",
                transition: "all .15s",
                marginBottom: 12,
              }}
            >
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: "none",
                  background: files.length > 0 ? "#2563eb" : "#e5e7eb",
        ...
        {state === "success" && results && (
          <>
            ...
            <p style={{ margin: "0 0 10px", fontSize: 12, color: "#6b7280", textAlign: "center" }}>
              {results.length} fichier{results.length > 1 ? "s" : ""} traité{results.length > 1 ? "s" : ""} — {totaux.total} ligne{totaux.total > 1 ? "s" : ""} au total
            </p>
